...
interface ProfileHeaderProps {
  profile: any
  isOwner: boolean
  stats?: { projects: number; upvotes: number; trackers?: number; signals?: number; requests?: number }
  isTracking?: boolean
  isLoggedIn?: boolean
}

export default function ProfileHeader({ profile, isOwner, stats, isTracking = false, isLoggedIn = false }: ProfileHeaderProps) {
    ...
    <div className="relative mb-12">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="relative -mt-16 sm:-mt-20 flex flex-col sm:flex-row items-center sm:items-end justify-between">
              ...
              <div className="flex flex-wrap justify-center sm:justify-start gap-4 sm:gap-6 sm:mb-4 mb-0">
                {profile.role === 'client' ? (
                  <div className="flex flex-col items-center sm:items-start">
                    ...
                    <span className="text-xs sm:text-sm font-medium text-foreground/50 uppercase tracking-wider mt-0.5">Requests</span>
                  </div>
                ) : (
                  <>
                    <div className="flex flex-col items-center sm:items-start">
                      <span className="text-xl sm:text-2xl font-extrabold text-white">{stats?.projects || 0}</span>
                      <span className="text-xs sm:text-sm font-medium text-foreground/50 uppercase tracking-wider mt-0.5">Projects</span>
                    </div>
                    <div className="flex flex-col items-center sm:items-start">
                      <span className="text-xl sm:text-2xl font-extrabold text-white">{stats?.upvotes || 0}</span>
                      <span className="text-xs sm:text-sm font-medium text-foreground/50 uppercase tracking-wider mt-0.5">Upvotes</span>
                    </div>
                  </>
                )}
                <div className="flex flex-col items-center sm:items-start">
                  <span className="text-xl sm:text-2xl font-extrabold text-white">{stats?.trackers || 0}</span>
                  <span className="text-xs sm:text-sm font-medium text-foreground/50 uppercase tracking-wider mt-0.5">Trackers</span>
                </div>
                <div className="flex flex-col items-center sm:items-start">
                  <span className="text-xl sm:text-2xl font-extrabold text-white">{stats?.signals || 0}</span>
                  <span className="text-xs sm:text-sm font-medium text-foreground/50 uppercase tracking-wider mt-0.5">Signals</span>
                </div>
              </div>
            </div>
            
            {/* Action buttons (Mobile only: positioned under avatar) */}
            <div className="w-full flex flex-wrap justify-center sm:hidden mt-3 gap-2">
              <button 
                onClick={handleShare}
              ...
              {isOwner ? (
                <button 
                  ...
                  className="px-5 py-2 rounded-full border border-transparent bg-white text-black hover:bg-white/90 text-sm font-bold transition-colors shadow-sm"
                >
                  Edit Profile
                </button>
              ) : (
          ...
          <div className="hidden sm:flex items-center gap-3">
            ...
            {isOwner ? (
              <button 
                onClick={() => setIsEditModalOpen(true)}
                className="px-6 py-2.5 rounded-full border border-transparent bg-white text-black hover:bg-white/90 font-bold transition-colors shadow-sm"
              >
                Edit Profile
              </button>
            ) : (
        ...
        <div className="mt-4 sm:mt-5 text-center sm:text-left flex flex-col items-center sm:items-start">
          ...
          <div className="mt-4 text-neutral-300 text-[15px] max-w-3xl leading-relaxed whitespace-pre-wrap break-words">
